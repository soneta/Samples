...
[assembly: ModuleType("LotyWidokowe", typeof(PrezentacjaGeekOut2019.LotyWidokowe.LotyWidokoweModule), 4, "LotyWidokowe", 3, VersionNumber=201)]

namespace PrezentacjaGeekOut2019.LotyWidokowe {

	/// <summary>
	/// Moduł LotyWidokowe.
	/// <seealso cref="Soneta.Config"/>
	/// <seealso cref="Soneta.CRM"/>
	/// <seealso cref="Soneta.Kasa"/>
	/// <seealso cref="Soneta.Business.Db"/>
	/// <seealso cref="Soneta.Business.Db.Permissions"/>
	...
	public partial class LotyWidokoweModule : Module {

		public static LotyWidokoweModule GetInstance(ISessionable session) => (LotyWidokoweModule)session?.Session?.Modules[moduleInfo];

		private static Soneta.Business.App.ModuleInfo moduleInfo = new Soneta.Business.App.ModuleInfo(session => new LotyWidokoweModule(session)) {
		};

		LotyWidokoweModule(Session session) : base(session) {}
		...
		private static Soneta.Business.App.KeyInfo keyInfoLotPrzezKod = new Soneta.Business.App.KeyInfo(tableInfoLoty, table => new LotTable.PrzezKodKey(table)) {
			Name = "PrzezKod",
			Unique = true,
			PrimaryKey = true,
		...
		public abstract partial class LotTable : GuidedTable {
			...
			public class PrzezKodKey : Key<Lot> {
				internal PrzezKodKey(Table table) : base(table) {
				}
			...
			public PrzezKodKey PrzezKod => (PrzezKodKey)Session.Keys[keyInfoLotPrzezKod];
			...
			/// </summary>
			/// <seealso cref="LotyWidokoweModule"/>
			public new LotyWidokoweModule Module => (LotyWidokoweModule)base.Module;
			...
			protected override sealed void PrepareNames(StringBuilder names, string divider) {
				...
				names.Append(divider); names.Append("Cena");
		...
		public abstract partial class LotRow : GuidedRow {
			...
			public int Cena {
				...
				set {
					...
					if (value==0) throw new RequiredException(this, "Cena");
					GetEdit(record==null, false);
					record.Cena = value;
			...
			[ChildTable("Rezerwacja", "PrezentacjaGeekOut2019.LotyWidokowe.Rezerwacja", "Lot")]
			...
			[Browsable(false)]
			public LotyWidokoweModule Module => Table.Module;
			...
			class CenaRequiredVerifier : RequiredVerifier {
				internal CenaRequiredVerifier(IRow row) : base(row, "Cena") {
				}
				protected override bool IsValid() => !(((LotRow)Row).Cena==0);
			}
			...
			protected override void OnAdded() {
				...
				Session.Verifiers.Add(new CenaRequiredVerifier(this));
		...
		public static class LotSchema {
			...
			internal static RowDelegate<LotRow, int> CenaBeforeEdit;
			public static void AddCenaBeforeEdit(RowDelegate<LotRow, int> value)
				=> CenaBeforeEdit = (RowDelegate<LotRow, int>)Delegate.Combine(CenaBeforeEdit, value);
		...
		private static Soneta.Business.App.KeyInfo keyInfoMaszynaPrzezNrBoczny = new Soneta.Business.App.KeyInfo(tableInfoMaszyny, table => new MaszynaTable.PrzezNrBocznyKey(table)) {
			Name = "PrzezNrBoczny",
			Unique = true,
			PrimaryKey = true,
		...
		/// <seealso cref="Soneta.Business.Table"/>
		[Caption("Tabela maszyn którymi mozna odbyć lot")]
		public abstract partial class MaszynaTable : GuidedTable {

			protected MaszynaTable() {}

			public class PrzezNrBocznyKey : Key<Maszyna> {
				internal PrzezNrBocznyKey(Table table) : base(table) {
				}
			...
			public PrzezNrBocznyKey PrzezNrBoczny => (PrzezNrBocznyKey)Session.Keys[keyInfoMaszynaPrzezNrBoczny];
			...
			protected override sealed void PrepareNames(StringBuilder names, string divider) {
				...
				names.Append(divider); names.Append("RokProd");
		...
		public abstract partial class MaszynaRow : GuidedRow {
			...
			public string NrBoczny {
				...
				set {
					...
					if (State!=RowState.Detached) {
						ResyncSet(keyInfoMaszynaPrzezNrBoczny);
					}
			...
			public string Producent {
				...
				set {
					...
					if (State!=RowState.Detached) {
						Session.Verifiers.Add(new Maszyna.WymaganaNazwa((Maszyna)this));
					}
			...
			public Date RokProd {
				get {
					if (record==null) GetRecord();
					return record.RokProd;
				}
				set {
					MaszynaSchema.RokProdBeforeEdit?.Invoke((Maszyna)this, ref value);
					if (value==Date.MinValue) throw new RequiredException(this, "RokProd");
					GetEdit(record==null, false);
					record.RokProd = value;
					MaszynaSchema.RokProdAfterEdit?.Invoke((Maszyna)this);
				}
			}
			...
			[ChildTable("Rezerwacja", "PrezentacjaGeekOut2019.LotyWidokowe.Rezerwacja", "Maszyna")]
			...
			class RokProdRequiredVerifier : RequiredVerifier {
				internal RokProdRequiredVerifier(IRow row) : base(row, "RokProd") {
				}
				protected override bool IsValid() => !(((MaszynaRow)Row).RokProd==Date.MinValue);
			}
			...
			protected override void OnAdded() {
				...
				Session.Verifiers.Add(new RokProdRequiredVerifier(this));
		...
		public static class MaszynaSchema {
			...
			internal static RowDelegate<MaszynaRow, Date> RokProdBeforeEdit;
			public static void AddRokProdBeforeEdit(RowDelegate<MaszynaRow, Date> value)
				=> RokProdBeforeEdit = (RowDelegate<MaszynaRow, Date>)Delegate.Combine(RokProdBeforeEdit, value);

			internal static RowDelegate<MaszynaRow> RokProdAfterEdit;
			public static void AddRokProdAfterEdit(RowDelegate<MaszynaRow> value)
				=> RokProdAfterEdit = (RowDelegate<MaszynaRow>)Delegate.Combine(RokProdAfterEdit, value);
		...
		private static Soneta.Business.App.KeyInfo keyInfoRezerwacjaPrzezNrRezerwacji = new Soneta.Business.App.KeyInfo(tableInfoRezerwacje, table => new RezerwacjaTable.PrzezNrRezerwacjiKey(table)) {
			Name = "PrzezNrRezerwacji",
			Unique = true,
			PrimaryKey = true,
		...
		public abstract partial class RezerwacjaTable : Table {
			...
			public class PrzezNrRezerwacjiKey : Key<Rezerwacja> {
				internal PrzezNrRezerwacjiKey(Table table) : base(table) {
				}
			...
			public PrzezNrRezerwacjiKey PrzezNrRezerwacji => (PrzezNrRezerwacjiKey)Session.Keys[keyInfoRezerwacjaPrzezNrRezerwacji];
		...
		public abstract partial class RezerwacjaRow : Row {
			...
			public string NrRezerwacji {
				...
				set {
					...
					if (State!=RowState.Detached) {
						ResyncSet(keyInfoRezerwacjaPrzezNrRezerwacji);
					}
	...
	[System.CodeDom.Compiler.GeneratedCode("Soneta.Generator", "4")]
	public static class StaticsLotyWidokoweModule {
		public static LotyWidokoweModule GetLotyWidokowe(this Session session) => LotyWidokoweModule.GetInstance(session);
	}
